...
def normalize_string(s):
    # print("unicode:{}".format(s))

    s = unicode_to_ascii(s.lower().strip())
    # print("ascii:{}".format(s))
    s = re.sub(r"([.!?])", r" \1", s)
...
# 把所有的单词全都转换成 数字的形式/index
# word2index
# To train we need to turn the sentences into something the neural network can understand,
# which of course means numbers. Each sentence will be split into words and turned into a Tensor,
# where each word is replaced with the index (from the Lang indexes made earlier).
# While creating these tensors we will also append the EOS token to signal that the sentence is over.
...
class BahdanauAttnDecoderRNN(nn.Module):
    ...
    def forward(self, word_input, last_hidden, encoder_outputs):
        # Note that we will only be running forward for a single
        # decoder time step, but will use all encoder outputs
...
# and apply those weights to the encoder outputs to get a context vector.
